...
| SMA8002 | Null suppression operation                 | Null suppression operation should be fenced with 3 parentheses to improve visual attention and text-based traceability. Strongly recommended that safely suppressing it by adding `Debug.Assert({0} is not null);` instead of `!`, without introducing runtime overhead in Release build.
