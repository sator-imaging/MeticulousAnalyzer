...
namespace SatorImaging.StaticMemberAnalyzer.Test
{
    ...
    {
        [TestMethod]
        public async Task FixAll_SMA8000()
        {
            var test = new VerifyCS.Test
            {
                CodeActionEquivalenceKey = "Use named argument",
            ...
            for (int p = 0; p < 3; p++)
            {
                ...
                for (int f = 0; f < 3; f++)
                {
                    ...
                    for (int i = 0; i < 3; i++)
                    {
                        var diag = VerifyCS.Diagnostic().WithLocation(path: fileName, line: 8 + i, column: new[] { 15, 21, 24 }[i]).WithArguments(arguments: new[] { "b", "i", "s" }[i]);
...
{{
    public class C
    {{
        public void M(bool b, int i, string s) {{ }}
        public void Test()
        {{
            M(true, 1, "");
        }}
