...
namespace Test
{
    public class CTest
    {
        ...
        public void Test()
        {
            var x = Foo({|#0:0|}, Bar(Baz(""message"", {|#1:2.2f|}), {|#2:11f|}));
        }
...
";
            ...
            var expected2 = VerifyCS.Diagnostic(ArgumentAnalyzer.RuleId_LiteralArgument).WithLocation(markupKey: 2).WithArguments("y");
            var expected3 = VerifyCS.Diagnostic(ArgumentAnalyzer.RuleId_LiteralArgument).WithLocation(markupKey: 3).WithArguments("a");
            var expected4 = VerifyCS.Diagnostic(ArgumentAnalyzer.RuleId_LiteralArgument).WithLocation(markupKey: 4).WithArguments("f");
            var expected5 = VerifyCS.Diagnostic(ArgumentAnalyzer.RuleId_LiteralArgument).WithLocation(markupKey: 5).WithArguments("y");
            await VerifyCS.VerifyCodeFixAsync(test, new[] { expected0, expected1, expected2, expected3, expected4, expected5 }, fixtest);
        }
    }
}
