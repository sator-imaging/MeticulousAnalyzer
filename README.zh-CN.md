...
- [Disposable 分析器](#disposable-分析器) 检测缺少 `using` 语句及更多
...
## 公开 API 中的 `Debug.Assert`

在公开 API 表面使用 `Debug.Assert` 会导致在 Release 构建中出现未定义的行为，因为 `Debug.Assert` 在 Release 构建中会被移除。请使用其他断言库或抛出异常。
