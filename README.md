...
> [!NOTE]
> `string`, `System.Text`, `System.IO`, or known testing framework methods and constructors are intentionally allowed. In addition, the first argument of type `string` or `char` can omit named argument. The first argument of type `int` can also omit named argument for method calls. Indexer arguments are also exempt from this analysis.
>
