...
> [!NOTE]
> ループ内の `yield return` および `yield break` 文は対象外です。また、ループ内で宣言されたローカル関数やラムダ式内部での大域脱出、直後に `return` または `throw` 文が続くループ、およびメソッドやループのルートレベルブロックの最後の文であるループでの大域脱出も対象外となります。
