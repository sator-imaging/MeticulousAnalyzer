...
比較演算や分岐条件の中でハードコードされたリテラル値（数値、0、文字列、文字）を直接使用することを回避します。定数や名前付き変数を使用して意図を明確にするか、末尾コメント `/* Why: 理由 */` を付与して抑制します。

```cs
if (status == 200) // 報告: 比較や分岐条件でのハードコードされたリテラルの使用を回避してください
{
    // ...
}

if (status == 200 /* Why: HTTP OK 標準ステータスコード */) // 許可: 末尾の抑制コメント
{
    // ...
}
```
...
> [!NOTE]
> コレクションや文字列の `Count`、`Length`、`IndexOf` プロパティ/メソッドとの比較、または `for`/`while`/`do-while` ループ条件ヘッダー内での `0` との比較は対象外です。
...
C# の `struct` 型に対して C++ スタイルのムーブセマンティクスを強制し、意図しないコピーや暗黙の破棄・共有を防ぎます。型名が `MoveOnly` で始まる（大文字小文字を区別）か、`[NoCopy]` 属性が付与されている型が MoveOnly 型として認識されます。
