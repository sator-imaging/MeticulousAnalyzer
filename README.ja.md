...
- SMA0080: Internal cross-namespace access
    ...
    - **例外**: アクセス先の末尾の名前空間（リーフ名前空間）が `Core` (ハードコード) または [設定](#アナライザーの設定方法) で指定された名前空間である場合は、名前空間をまたいだアクセスが許可されます。
    - `SR` という名前の型 (ハードコード) または [設定](#アナライザーの設定方法) で指定された型に定義されているメンバーも、このルールの対象外となります。
