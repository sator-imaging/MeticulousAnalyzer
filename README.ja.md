...
for (int i = 0; i < items.Length; i++)
{
    ...
    {
        return i;
        ~~~~~~ // エラー (SMA8032): ループからの大域脱出を回避してください。
    }
}
```

> [!TIP]
> コメント `// Allow non-local exit from loop` により抑制できます。詳細は [コメントによる抑制](#コメントによる抑制) セクションを参照してください
...
- **SMA0097**: MoveOnly 型を `Move()` メソッド以外で値として返すことはできません。`return` 文で `Move()` を呼び出した場合も禁止されますが、参照戻り値は許可されます。
...
- **SMA0033**: 非 `readonly` 構造体を `in` 引数として渡すと防衛的コピー（Defensive Copy）が発生します。
- **SMA0034**: 非 `readonly` 構造体の変数または戻り値を `ref readonly` として宣言すると防衛的コピー（Defensive Copy）が発生します。
