...
> [!NOTE]
> ループ内の `yield return` および `yield break` 文、ループ内で宣言されたローカル関数やラムダ式内部での大域脱出は対象外です。大域脱出が制御フローの最後の文の場合は、直後に `return` または `throw` 文が続くループ、またはメソッドやループのルートレベルブロックの最後の文であるループでの大域脱出も対象外となります。
