...
処理途中での分岐を禁止します。早期 return は問題ありませんが、メインフローの途中で新たな制御フロー分岐を持ち込まないでください。また、早期分岐ブロック（早期 return や早期 continue など）の中では、脱出前に状態変更を行う処理を含めないでください（`out` または `ref` 引数への代入は許可されます）。
