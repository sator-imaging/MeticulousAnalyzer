...
>
> なお、`null` や `default` リテラル、および boolean 式（パターンマッチングを含む。例: `foo is not null` や `x == y`）は、その位置や含まれる名前空間に関係なく、名前付き引数の省略対象外であり、常に名前を指定する必要があります。ただし、メソッド名が "true" または "false" で終わる（大文字小文字を区別しない）場合、boolean パラメーターの名前付き引数は省略可能です。
