...
";
    ...
    // Wrap generated method bodies in unchecked blocks
    var methodDeclRegex = new Regex(@"(\b(public|protected|internal|private)\s+[^;{}()]+?\([^)]*\)\s*)\{");
    ...
    for (int m = matches.Count - 1; m >= 0; m--)
    {
        ...
        if (closeIdx != -1)
        {
            string body = generatedCode.Substring(openIdx + 1, closeIdx - (openIdx + 1));
            string trimmedBody = body.TrimStart('\r', '\n');
            string suffix = trimmedBody.EndsWith("\n") ? "" : "\n";
            generatedCode = generatedCode.Substring(0, openIdx + 1)
                + "\nunchecked\n{\n"
                + trimmedBody
                + suffix
                + "}\n"
                + generatedCode.Substring(closeIdx);
        }
    }
